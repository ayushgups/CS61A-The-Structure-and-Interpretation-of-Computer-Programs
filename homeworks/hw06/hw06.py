...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.stock -= 1
            self.money -= self.money
            return f'Here is your {self.item} and ${var - self.price} change.'

    def add_funds(self, deposit):
        if self.stock == 0:
            return f'Nothing left to vend. Please restock. Here is your ${deposit}.'
        else:
            self.money += deposit
            return f'Current balance: ${self.money}'
    
    def restock(self, added_items):
        self.stock += added_items
...
def deep_map_mut(func, lnk):
    ...
    if isinstance(lnk.first, int):
        ...
        if isinstance(lnk.first.first, int):
...
head1 = Link(1, Link(2, Link(3, Link(4, Link(5, Link(6))))))
print(middle_node(head))
print(middle_node(head1))
