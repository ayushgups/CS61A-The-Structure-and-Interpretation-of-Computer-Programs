...
def merge(lst1, lst2):
    ...
    True
    """
    #taking the smallest element of one of the lists and merging the rest of the smaller list(using splicing) and the other list
    if len(lst1) == 0:
        return lst2
    if len(lst2) == 0:
        return lst1
...
def summation(n, term):
    ...
    """
    assert n >= 1
    def helper(i=1, total=0):
        total = term(i)
        if i > n:
            return 0
        else:
            return total + helper(i + 1)
