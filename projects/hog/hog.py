...
def always_roll(n):
    ...
    assert n >= 0 and n <= 10
    # BEGIN PROBLEM 6
    def always_roll_xy(x, y):
        return n
    return always_roll_xy
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while iterate < goal:
        iterate1 = 0
        while iterate1 < goal:
            if check != strategy(iterate, iterate1):
                return False
            iterate1 += 1
        iterate += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    bestroll = 0
    while num_rolls < 11:
        value = make_averaged(roll_dice, samples_count)
        max = value(num_rolls, dice)
        if highest < max:
            highest = max
            bestroll = num_rolls
        num_rolls += 1
