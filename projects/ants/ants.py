...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            elif isinstance(place.ant, ContainerAnt) and not place.ant.can_contain(self):
                assert place.ant is None, 'Two ants in {0}'.format(place)
            elif isinstance(self, ContainerAnt) and not self.can_contain(place.ant):
                assert place.ant is None, 'Two ants in {0}'.format(place)
            elif isinstance(self, ContainerAnt) and not isinstance(place.ant, ContainerAnt):
                self.store_ant(place.ant)
                place.ant = self
            elif not isinstance(self, ContainerAnt) and isinstance(place.ant, ContainerAnt):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, lower_bound=0, upper_bound=float('inf')):
        ...
        location = self.place
        count = 0
        while location.is_hive != True and count <= self.upper_bound:
            if location.bees != []:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        list_of_bees = list(current_place.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            ...
            if self.place.bees:
                randbee = random_bee(self.place.bees)
                randbee.reduce_health(randbee.health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        if isinstance(other, ContainerAnt) or self.ant_contained:
            return False
        else:
            return True
        # END Problem 8a
