...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new_paragraphs = [i for i in paragraphs if select(i) == True]
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    if min([diff_function(typed_word, i, limit) for i in word_list]) > limit:
        return typed_word
    list = [diff_function(typed_word, i, limit) for i in word_list]
    return word_list[list.index(min(list))] 
...
def feline_fixes(typed, source, limit):
    ...
    if len(typed) == 0 and len(source) == 0:
        ...
        return abs(len(typed) - len(source)) + feline_fixes(typed[-1:0], source[-1:0], limit)
...
def time_per_word(words, timestamps_per_player):
    ...
    for timestamps in timestamps_per_player:
        ...
        for j in range(1, len(timestamps)):
            player_times.append(timestamps[j] - timestamps[j-1])
        times_list.append(player_times)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    players = len(get_all_times(match))
    num_words = len(get_all_words(match))
    fastest_for_each_player = [[] for _ in range(players)]
    for word_index in range(num_words):
        quickest_player = 0
        quickest_player_time = time(match, 0, word_index)
        for player_num in range(1, players):
            if time(match, player_num, word_index) < quickest_player_time:
                quickest_player_time = time(match, player_num, word_index)
                quickest_player = player_num
        
        fastest_for_each_player[quickest_player].append(get_word(match, word_index))
    
    return fastest_for_each_player
    # END PROBLEM 10
