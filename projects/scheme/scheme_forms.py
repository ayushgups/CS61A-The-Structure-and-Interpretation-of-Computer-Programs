...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        evaluated_operand = scheme_eval(operand.first, env)
        x = env.define(operator, evaluated_operand)
        return operator
        # END PROBLEM 4
        ...
        body = Pair(formals, expressions.rest)
        lbd = do_lambda_form(body, env)
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    if expressions == nil:
        return is_scheme_true(scheme_eval(expressions, env))
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    if expressions == nil:
        return is_scheme_false(scheme_eval(expressions, env))
    while expressions.rest != nil:
        if is_scheme_true(scheme_eval(expressions.first, env)):
            return scheme_eval(expressions.first, env)
        else:
